  ...
  function detectOverviewMediaKind(item){
    ...
    const aliases = {
      photo: "image", picture: "image", pic: "image",
      voice: "audio", voice_note: "audio", voicenote: "audio",
      document: "file", attachment: "file",
    };
    if (["image", "video", "audio", "file"].includes(explicitKind)) return explicitKind;
    if (aliases[explicitKind]) return aliases[explicitKind];
    ...
    const fromName = inferMediaKind(item.media_name || "", "");
  ...
  function renderChatInfoTab(){
    ...
    const mediaOnlyRaw = media.filter((m)=> {
      const kind = detectOverviewMediaKind(m);
      return kind === "image" || kind === "video";
    });
    const mediaOnly = mediaOnlyRaw.length ? mediaOnlyRaw : media.filter((m)=> detectOverviewMediaKind(m) !== "audio");
